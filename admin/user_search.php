...
if(!$USER->CanDoOperation('view_all_users'))
{
	...
	if($FN == 'sale_order_create_form' || $FN == 'find_form') {
        $arUserSubordinateGroups = array_merge($arUserSubordinateGroups, $arUserGroups);
    }
...
foreach($dbUsersOrderData as $userdata)
{
    $userOrderData[$userdata['USER_ID']]=$userdata;
}
...
{
	$f_ID = $arRes['ID'];
	$formattedPrice=($userOrderData[$f_ID]["SUM_PAID"]>0)?SaleFormatCurrency($userOrderData[$f_ID]["SUM_PAID"], $userOrderData[$f_ID]["CURRENCY"]):0;
	$formattedCount=$userOrderData[$f_ID]["COUNT_FULL_PAID_ORDER"]?$userOrderData[$f_ID]["COUNT_FULL_PAID_ORDER"]:0;
	...
	$row->AddViewField("COUNT_FULL_PAID_ORDER", $formattedCount);
	$row->AddViewField("SUM_PAID", $formattedPrice);
