...
class CModuleOptions
{
	...
	private function SaveOptions()
	{
		...
		{
			if($arOptParams['TYPE'] != 'CUSTOM' && $arOptParams['TYPE'] != 'INFO')
			{
				$val = $_REQUEST[$opt];
	...
	public function ShowHTML()
	{
		...
		if(is_array($this->arOptions))
		{
			foreach($this->arOptions as $option => $arOptParams)
			{
				...
				{
					case 'DATE':
						$input = '<div class="adm-input-wrap adm-input-wrap-calendar">
									<input class="adm-input adm-input-calendar" type="text" name="'.$opt.'" size="22" value="'.$val.'">
									<span class="adm-calendar-icon" title="Нажмите для выбора даты" onclick="BX.calendar({node:this, field:\''.$opt.'\', form: \'\', bTime: false, bHideTime: true});"></span>
								</div>';
						break;
					case 'CHECKBOX':
					...
					case 'CUSTOM':
						...
						break;
					case 'INFO':
						$input = '<tr><td colspan="2"><div class="info" style="background-color: #FEFDEA;border: 1px dotted #D7D6BA;margin: 16px 50px;padding: 10px;font-size: 13px;">'.$arOptParams['VALUE'].'</div></td></tr>';
						break;
					default:
